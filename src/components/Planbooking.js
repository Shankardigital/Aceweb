...
const PlanBooking = () => {
    ...
    const batchdet = sessionStorage.getItem('batchdata');
    const batchdetails = JSON.parse(batchdet);
    ...
    let bookinglength;
    if (plans.type === "Team_subcription") {
        bookinglength = bookingdata.length;
    ...
    const counts = plans.price

    let Sessions;
    if (plans.type === "Team_subcription" || plans.type === "Membership") {
        Sessions = "One Month";
    } else if (plans.type === "Classes") {
        Sessions = batchdetails.days;
    } else if (plans.type === "Clinics" || plans.type === "PayasYouGo") {
        Sessions = "-";
    }

    // const apicalls = plans.type === "Membership" ? addteamsubscription : addmembership
    // console.log(apicalls)


    //  else if (plans.type === "PayasYouGo") {
    //   apicalls = "addmembership";
    // }
    ...
    if (getcoupon.length === 0) {
        total = parseFloat(percentage) + parseFloat(plans.price);
    }
    else {
        total = parseFloat(percentage) + parseFloat(plans.price) - couponvalue;
    }
    ...
    const addbookings = (paymentdata) => {
        let apicalls;
        if (plans.type === "Team_subcription") {
            apicalls = "addteamsubscription";
        } else if (plans.type === "Membership") {
            apicalls = "addmembership";
        } else if (plans.type === "Classes") {
            apicalls = "addmyclass";
        } else if (plans.type === "Clinics") {
            apicalls = "addclinic";
        } else if (plans.type === "PayasYouGo") {
            apicalls = "addpayasyougo";
        }
        console.log(apicalls)
        ...
        const mydata = {
            ...
            batchId: plans.type === "Classes" ? batchdetails._id : "",
            teamMembers: plans.type === "Team_subcription" ? bookingdata : "",
        ...
        axios.post(URL[apicalls], mydata,
            {
                headers: { Authorization: `Bearer ${accessToken}` }
            }
        ).then((res) => {
                                    ...
                                    <div className="row text-dark">
                                        ...
                                        <div className="col">
                                            <div style={{ direction: "rtl" }}>
                                                ...
                                                <p className="text-dark">{Sessions}</p>
                                                {plans.type === "Team_subcription" ? (

                                                    <p className="text-dark">{bookinglength}</p>
                                                ) : (
